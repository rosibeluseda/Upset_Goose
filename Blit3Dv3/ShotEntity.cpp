...
ShotEntity * MakeShot()
{
	...
	boxShape.SetAsBox(160.0f / (2.f * PTM_RATIO), 110.0f / (2.f * PTM_RATIO));
	...
	fixtureDef.filter.maskBits = CMASK_SHOT | CMASK_EDGES | CMASK_BLOCK | CMASK_GROUND | CMASK_ALIEN;//it collides wth lots of stuff

	// Define the dynamic body fixture.
	//fixtureDef.shape = &dynamicShot; &boxShape;
	fixtureDef.shape = &boxShape; ////ROSI
...
ShotEntity* MakeShot()
{
	...
	//A circle shape for our shot
	b2CircleShape dynamicShot;
	dynamicShot.m_radius = 80.f / (2 * PTM_RATIO); //shot has diameter of 9 pixels
